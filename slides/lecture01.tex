...
\begin{itemize}
\item ``Question testing'' split ballots (e.g., Cantril)
...
\frame{
	...
	\begin{itemize}\itemsep0.5em
		...
		\item<2-> We can average: $E[TE] = E[Y_{1} - Y_{0}] = E[Y_{1}] - E[Y_{0}]$
...
$\widehat{Var}(SATE) = \widehat{Var}\left(\bar{Y}_0\right) + \widehat{Var}\left(\bar{Y}_1\right)$

	\begin{itemize}
	\item $\widehat{Var}(\bar{Y}_0)$ is control group variance
	\item $\widehat{Var}(\bar{Y}_1)$ is treatment group variance
	\end{itemize}

\item We often express this as the \textit{standard error} of the estimate:\\
$\widehat{SE}_{SATE} = \sqrt{\widehat{Var}\left(\bar{Y}_0\right) + \widehat{Var}\left(\bar{Y}_1\right)}$
\end{itemize}
}
