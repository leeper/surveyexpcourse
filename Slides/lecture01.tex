...
\item RCTs came later to medicine (circa 1950)
...
\item First experiment by Gosnell (1924)
...
\frametitle{{\large In Political Science II}}
...
\item<3-> Mid-1980s: Paul Sniderman \& Tom Piazza performed the first survey experiment\footnote{Sniderman, Paul M., and Thomas Piazza. 1993. \textit{The Scar of Race}. Cambridge, MA: Harvard University Press.}
	\begin{itemize}\footnotesize
...
\only<3->{\textbf{Treatment}: An intervention, whose effects we wish to assess relative to some other (non-)intervention}
...
\item<5-> Basically: $Y = \beta_0 + \beta_1 X + \epsilon $
\end{itemize}
}
...
If we are interested in some outcome $Y$, then for every unit $i$, there are numerous ``potential outcomes'' $Y*$ only one of which is visible in a given reality. Comparisons of (unobservable) potential outcomes indicate causality.
...
\item Reasons to choose one procedure over another:
	\begin{itemize}
	\item Disciplinary norms
	\item Ease of interpretation
	\item Flexibility for >2 treatment conditions
	\end{itemize}
\end{itemize}
...
lm(outcomes ~ treatment, data = data)
\end{verbatim}
